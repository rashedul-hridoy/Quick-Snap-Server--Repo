...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.akesmbp.mongodb.net/?retryWrites=true&w=majority`;
...
async function run(){
    try{
        ...
        app.get('/myreviews/:id', verifyJWT,  async (req, res) => {
            ...
            const id = req.params.id;
            const query = { _id: ObjectId(id) };
            const review = await reviewCollection.findOne(query).sort( { time : -1 } );
            res.send(review);
        });
